...
class GroupRandomColorjitter(object):
    ...
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0):
        ...
        # print (self.brightness, self.contrast, self.saturation, self.hue) # [0, 2] [0, 2] [0, 2] [-0.05, 0.05]
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        # print (brightness_factor, contrast_factor, saturation_factor)
    ...
    def __call__(self, img_group):
        """
        Args:
            img_group (PIL Image): Input image.
        Returns:
            PIL Image: Color jittered image.
        """
        transform = self.get_params(self.brightness, self.contrast,
                                    self.saturation, self.hue)
        ret = [transform(img) for img in img_group]
        return ret
...
class GroupRandomRotation(object):
    ...
    def __call__(self, img_group):
        # theta = np.pi / 180 * rnd.uniform(-self.rt, self.rt)
        theta = random.uniform(-self.rt, self.rt)
...
def channel_shift(xs, intensity, channel_axis):
    ...
    xs = channel_shift(xs,
                        intensity,
                        img_channel_axis)
class GroupRandomColorjitter_km(object):
    def __init__(self, alpha=0.1, cs=5):
        self.alpha = alpha
        self.cs = cs

    @staticmethod
    def get_params(alpha, cs):
    ...
    def __call__(self, img_group):
        ...
        scale, intensity = self.get_params(self.alpha, self.cs)
        for img in img_group:
        ret = [img.rotate(theta) for img in img_group]
        return ret
'''
